...
%% Reactions
    ...
    % Remove bibliomic genes not present in the model
    tableRxns(~ismember(tableRxns.rxnID, genericModel.rxns), :) = [];
...
for j = 1:length(models)
    ...
    activeRxns = tableRxnsExcluded.rxnID;
    ...
    % Save figure
%     savefig([pathSave filesep models{j} filesep 'bibliomicsGenesRxns'])
%     saveas(gcf,[pathSave filesep models{j} filesep 'bibliomicsGenesRxns'],'png')
%     saveas(gcf,[pathSave filesep models{j} filesep 'bibliomicsGenesRxns'],'eps')
    
    clearvars -except j models tableRxns tableGenes pathSave
end